...
import com.py.projects.trello.controllers.InvalidatedTokenCollection;
...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (requestTokenHeader != null && requestTokenHeader.startsWith("Bearer ")) {
            ...
            try {
                if (invalidatedTokenCollection.hasToken(jwtToken)) {
                    throw new InvalidatedJwtException("The token used has already been invalidated");
                }
        ...
        if (emailAddress != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            UserDetails userDetails = this.jwtUserDetailsService.loadUserByUsername(emailAddress);
